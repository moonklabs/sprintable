"""story #3589(Phase2·BE·소형·결함, 페드루 PO 確定 2026-09-06) — 영상/이미지
confirm이 422로 거부되면 업로드된 GCS 객체가 고아로 남던 결함(「용량 초과」
갈래 하나에만 `delete_object`가 있어 나머지 거부 갈래가 전부 샘 — 유나 배포
43 회차 실측: 이미지 2장 초안에 영상 PUT 200→confirm 422여도 스토리지엔
객체가 그대로 남았다).

처방(channel_post_videos.py::confirm_channel_post_video_upload·channel_post_
images.py::confirm_channel_post_image_upload) — head_object 확認 뒤 구간
전체를 try/except 하나로 감싸 갈래별 delete_object 누락을 구조적으로 없앴다.
이 파일은 그 처방을 실측한다: 거부 갈래마다 (a) 디스크에서 객체가 실제로
사라졌는지(LocalStorageProvider·head_object) (b) delete_object가 정확히
1회만 불렸는지(클래스 메서드 카운팅 래핑) — 성공 갈래는 둘 다 0.

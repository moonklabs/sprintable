...
게이트) AC1~AC5(submit-time 거부·재승인·발행 직전 재검사·evidence 지출 합산).
블루프린트 v3 §2 「생성 비용 한도」·§PO-3 댄 걸린 자리 4·10.

story #3619(CI·소형, 페드루 PO 確定 2026-09-07)로 조각①(GET /generation-budget)·
조각⑤(evidence payload 검증)·PUT 3종 검증은 test_3498_generation_budget_evidence_
and_config.py로 분리했다 — 파일이 이미 갖고 있던 8절 구분 경계를 그대로 써서
절반으로 갈랐다(그 파일 docstring에 원인 실측 상세). 이 파일엔 app.main import
유무로 갈리는 명확한 두 그룹이 없어(로컬 --durations 실측 — 전 테스트가 고르게
0.3~0.5s대) #3502처럼 "무거운 조각만 뽑기"가 아니라 기존 절 경계로 단순 이분했다.

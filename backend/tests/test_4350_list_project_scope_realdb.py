...
async def _seed(s):
    ...
    await s.commit()
    owner = uuid.uuid4()  # org owner — 전체 접근(까디르 N5 양성 대조)
    s.add(User(id=owner, email=f"o-{owner.hex[:8]}@test.com", hashed_password="x"))
    await s.commit()
    s.add(OrgMember(id=uuid.uuid4(), org_id=org.id, user_id=owner, role="owner"))
    await s.commit()
    return {
        "org": org.id, "pa": pa.id, "pb": pb.id, "user": uid, "agent": agent_id, "nobody": nobody, "owner": owner,
...
async def _get(Session, seeded, path, caller, *, expect_status=200, want_headers=False):
    ...
    async def _auth():
        ...
        return AuthContext(
            user_id=str(seeded[caller if caller in ("nobody", "owner") else "user"]), email="h@test",
...
@pytest.mark.parametrize("path", [
    "/api/v2/stories?project_id={pb}",
    "/api/v2/goals?project_id={pb}",
    # 까디르 4721 [P2] — ids= 분기가 project_id 확인을 건너뛰면 안 된다(예전 goals는 ids 분기가 먼저 반환해 200).
    "/api/v2/stories?ids={story_b},{story_a}&project_id={pb}",
    "/api/v2/goals?ids={goal_b},{goal_a}&project_id={pb}",
])
async def test_explicit_inaccessible_project_is_rejected(path):
    """접근 불가 project_id 명시 = 거절(403/404만 — `200 []`는 통과가 아니다 · 까디르 4721 [P2]) · 본문에 그 프로젝트 항목 0."""
    engine, Session = await _session_factory()
    try:
        async with Session() as s:
            seeded = await _seed(s)
        resp = await _get(Session, seeded, path.format(**seeded), "human", expect_status=None)
...
async def test_org_owner_sees_every_project(path, own, other):
    """까디르 4721 [P3] N5 — owner/admin(org 전체 접근)은 옛 동작 그대로 두 프로젝트 모두."""
    engine, Session = await _session_factory()
    try:
        async with Session() as s:
            seeded = await _seed(s)
        ids = await _get(Session, seeded, path, "owner")
        assert {seeded[own], seeded[other]} <= ids
    finally:
        await engine.dispose()

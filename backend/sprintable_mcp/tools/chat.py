...
class SendChatInput(ConversationScopedInput):
    ...
    # story #4329 — `message_type` · `review_type` · `metadata`는 뺐다: MCP가 `metadata`로 묶어 보냈지만 BE `SendMessageRequest`에
    # 그 필드가 없어 조용히 버려졌다(저장 · 표시된 적 없음). 저장이 필요한 쓰임이 생기면 별도 카드(PO 결정 · API 표면 넓히지 않음).

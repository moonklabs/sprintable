...
class DualPublishEventBroker:
    ...
    async def publish_atomic(
        ...
        """이 구현체엔 outbox가 없다 — `publish()`로 graceful degrade(비-atomic, 즉시 발행).
        `db`는 미사용(Protocol 정합용) — 진짜 atomic 보장은 `OutboxEventBroker.publish_atomic()`
        만 제공한다(모듈 싱글턴 `event_broker`가 실제로 쓰는 구현체)."""
        await self.publish(target, target_id, event_type, data)
...
class OutboxEventBroker:
    """E-ARCH S3 구현체 — `DualPublishEventBroker`(PG NOTIFY+Redis shadow) 위에
    `event_outbox` row insert를 추가한다.

    두 갈래 API:
    - `publish()`(3a, 기존): 호출 타이밍 무변경(여전히 caller commit 이후, 별도 짧은
      트랜잭션) — 20개 콜사이트 전부가 여전히 이걸 쓴다. 아직 진짜 atomic이 아니다.
    - `publish_atomic()`(3b, story #2078 SSE 좁힘 설계 2026-07-21): caller가 자신의 `db`
      세션을 넘겨 outbox row를 **caller의 commit과 같은 트랜잭션**에 싣는다(여기선 commit
      안 함 — caller가 자기 commit에 포함). "state 커밋됐는데 event가 안 나가는" 갭을 완전히
      닫는다. 스코프는 SSE 신뢰성만(오르테가군 판정 — 5-effect 전부가 아니라 SSE 1개만
      atomic이면 목표(emit 누락 0) 달성) — webhook/L2/notification/StoryActivity 등 나머지
      side-effect는 여전히 기존 post-commit best-effort 경로 그대로.

    `event_broker_outbox_enabled`(default False)로 둘 다 게이트 — 꺼져 있으면 어느 쪽도
    insert 자체를 시도하지 않는다(무회귀). 콜사이트가 `publish_atomic`으로 이관해도, 플래그가
    꺼져 있는 동안은 그 콜사이트의 **기존** post-commit 실 dispatch(예: story_status_events.py
    의 `_push_to_agent` 루프)가 여전히 유일한 실 경로로 남아있어야 한다(둘 다 켜진 상태에서
    플래그를 켜면 중복 dispatch — 콜사이트 이관 완료 후 기존 루프 제거 + 플래그 on이 동시
    cutover여야 안전, 오늘 LISTEN+Redis 공존 버그와 동형 위험).
    """
    ...
    async def publish_atomic(
        ...
        """caller의 트랜잭션에 outbox row를 싣는다 — commit은 caller 책임(여기선 안 함).

        `event_broker_outbox_enabled`가 꺼져 있으면 완전 no-op(row 자체를 안 만든다) — 콜사이트가
        미리 이관해둬도 플래그 off인 동안은 무해(기존 경로가 유일 실 dispatch로 계속 동작).
        org_id 해소 실패(best-effort, #2075 UNION 패턴 재사용)는 경고 로그만 남기고 조용히 skip —
        outbox insert 실패가 caller의 state 커밋 자체를 막으면 안 된다(3b도 "부가" 가드).
        """
        from app.core.config import settings

        if not settings.event_broker_outbox_enabled:
            return

        org_id = await _resolve_org_id(target, target_id, data)
        if org_id is None:
            logger.warning(
                "event_broker publish_atomic skipped (org_id unresolved) target=%s target_id=%s event_type=%s",
                target, target_id, event_type,
            )
            return

...
2. **PO 헤더는 영문만** — ``## PO review — PASS`` / ``## PO review — CHANGES``(대소문자
   구분 — `_PO_REVIEW_RE`는 IGNORECASE 플래그가 없다). 페드루 자신의 헤더 규율 고정
   (2026-09-16 16:33Z, "앞으로 `## PO review — PASS|CHANGES` 고정") 이후로 옛 한글
   「PO 리뷰」 헤더가 이 웹훅(`action=created` 새 코멘트만 봄) 경로로 도달할 일 자체가
   없다 — 지원 안 함이 아니라 사실상 재발 경로가 없어진 것.

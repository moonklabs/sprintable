...
RESERVED_WORKSPACE_SLUGS: frozenset[str] = frozenset({
    ...
    # `tests/test_4218_reserved_workspace_slugs_sync.py`가 FE 목록을 직접 읽어 «FE ⊆ 여기»를 매 CI에서 잡는다 — FE에
    # 라우트가 늘면 그 테스트가 RED이니 여기에 더하면 된다. 형식상 slug가 될 수 없는 이름(`.well-known`·`favicon.ico` 등)도

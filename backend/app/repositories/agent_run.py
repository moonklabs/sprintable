...
class AgentRunRepository:
    ...
    async def list(
        ...
        conds = [AgentRun.project_id == project_id]
        if agent_id is not None:
            conds.append(AgentRun.agent_id == agent_id)
        ...
        if story_id is not None:
            conds.append(AgentRun.story_id == story_id)
        # story #3680 — status/from/to 전부 narrowing(AND)뿐, story_id와 동형(인가 축 신규 0).
        if status is not None:
            conds.append(AgentRun.status == status)
        if from_dt is not None:
            conds.append(AgentRun.created_at >= from_dt)
        if to_dt is not None:
            conds.append(AgentRun.created_at <= to_dt)
        # story #3851 — total = 위 필터 전부(+cursor) 適用 後 남은 개수(base.py 관례,
        # story #3841에서 이 코드베이스가 이미 고정한 것과 동형 — cursor 前 grand total이
        # 아니다). cursor 자체는 이 total 대상에도 포함해야 "남은 페이지 수"가 맞는다.
        if cursor:
            conds.append(AgentRun.created_at < cursor)

        count_result = await self.session.execute(
            select(func.count()).select_from(AgentRun).where(*conds)
        )

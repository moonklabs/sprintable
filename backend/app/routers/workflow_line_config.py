...
class VersionResponse(BaseModel):
    ...
    # S34 editor: draft 로드(편집)·version-history 표시용 additive 필드(하위호환 default).
    config: dict[str, Any] = {}
...
async def list_versions_endpoint(
    ...
    """⭐S34 editor version-history: scope(org/project)+entity_type 의 전 버전 목록(version desc).
    admin/draft-author 게이팅(create_draft 동일 권한·read-only)."""
    if entity_type not in ENTITY_TYPES:
        raise HTTPException(
            status_code=422, detail=f"entity_type must be one of {sorted(ENTITY_TYPES)}"
        )
    await _require_draft_author(session, uuid.UUID(auth.user_id), org_id, project_id)
    rows = await list_versions(session, org_id, project_id, entity_type)
...
async def update_draft_version(
    ...
    """⭐S34 editor "저장": draft config in-place 갱신(status=='draft' 만·published/approved 등 422 immutable·
    config_hash+lint 재계산). admin/draft-author 게이팅. 수정=새 draft 가 아니라 in-place(draft 증식 방지)."""

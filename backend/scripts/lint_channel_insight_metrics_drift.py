...
`EXPECTED_BACKEND_CHANNELS`(19개, 아래) 전부를 커버하는지 매 호출마다 확認하고, 하나라도
...
# channel_adapters.py에 실제로 등록된 채널 19개(2026-09-12 실측, story #3815 PR1·
# #3816 PR1 갱신 — grep으로 재확認 가능: `grep -c 'ChannelAdapterConfig(' backend/app/
# services/channel_adapters.py`). 이 집합
...
EXPECTED_BACKEND_CHANNELS = frozenset({
    ...
    "x", "x_sandbox",
    # story #3813(Phase3·3-4 PR1, 페드루 PO 確定 2026-09-12) — stibee/stibee_sandbox도
    # 같은 이유로 이 PR 시점엔 insight_metrics 미선언(빈 튜플 기본값) — 발송 결과
    ...
    # 엔드포인트가 없다, 그라운딩 확認 PO ⑥ "미측정"). x/x_sandbox·stibee/
    # stibee_sandbox와 동형으로 drift 0(신규 FE 등재 불요).
    "ghost", "ghost_sandbox",
})
...
def find_drift(
    ...
    에선 EXPECTED_BACKEND_CHANNELS(19개) 기본값을 쓰고, 합성 fixture 테스트는 그 fixture가
    실제로 선언한 채널 집합을 넘겨 좁힌다(19개 전부를 요구하면 모든 합성 테스트가 매번
    19채널을 다 적어야 해 테스트 자체가 무거워진다)."""

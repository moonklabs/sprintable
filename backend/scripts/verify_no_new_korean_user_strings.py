...
- **EXEMPT_FILES(이름 붙은 예외, story #3786 페드루 PO 明示 2026-09-10 12:24Z + story
  #4120 추가 2026-09-21)** — `app/services/i18n_catalog.py`(원 예외). 무배제 원칙은
  "코드에 박힌 하드코딩 한글"을 겨냥한 것이지, 이 파일처럼 ko/en 키를 **정본으로 보관하는
  자리**는 애초에 "하드코딩"이 아니다(반대로 세면 ①http_detail류 슬라이스가 문자열을
  라우터에서 이 카탈로그로 "옮길" 때마다 1층 총량이 안 줄어 진짜 진척(2층 도달가능 수)과
  1층 숫자가 영영 어긋나고, ②카탈로그에 새 키를 추가할 때마다 이 가드가 "신규 한글"로
  계속 RED가 난다 — 카탈로그의 존재 이유 자체와 이 가드가 구조적으로 충돌). 그 파일의
  ko/en 짝 무결성은 별도 가드(`test_3786_i18n_catalog.py`의 가드 1·2)가 이미 지킨다.
  `app/utils/korean_particle.py`(story #4120 추가) — `pick_i_ga_josa`의 반환값 `"이"`/
  `"가"`는 산문이 아니라 조사 낱말표(FE `korean-particle.ts` pickIGaJosa의 반환값과
  동형·같은 근거)라 i18n_catalog.py와 같은 클래스: 이 가드가 키우려는 "baseline은 절대
  안 는다" 계약과, 이 낱말표가 늘 때마다(새 조사 함수 추가 등) baseline이 함께 늘어야
  하는 구조가 충돌한다. 이 파일 하나를 빼도 "한글이 코드에 새로 박히는 것"을 못 잡는
  구멍이 생기지 않는다(단위 테스트 `tests/test_4120_korean_particle.py`가 반환값
  정확성을 직접 검증). 다른 모든 파일은 여전히 EXEMPT 0(이 예외들이 무배제 원칙 자체를
  깨는 것이 아니다 — story #3779 카드 본문에도 이 줄을 남긴다).
...
EXEMPT_FILES = frozenset({"app/services/i18n_catalog.py", "app/utils/korean_particle.py"})

...
(=본문 참조) 또는 'created_from'(=출처) 둘 뿐 — PG 버전 의존 0.

⛔"마이그 0"이 깨지는가: `ADD COLUMN ... NOT NULL DEFAULT 'none'`은 PG11+에서 상수 기본값
이라 **메타데이터 연산만으로**(테이블 재작성 0·별도 배치 UPDATE 불요) 기존 행 전부에 'none'을
채운다 — 로컬 PG로 실측: 138행 표에서 1.1ms. 데이터가 실제로 채워지므로 엄밀히는 "데이터
마이그레이션 0"은 아니지만, 이 경로는 행 수와 무관하게 항상 즉시(대상 표가 prod에서 지금보다
커도 위험이 없다) — "마이그 0"은 목표가 아니라 결과였을 뿐, 이 경로는 그 결과를 다른 방식
(사람이 쓴 UPDATE 없음)으로 달성한다.

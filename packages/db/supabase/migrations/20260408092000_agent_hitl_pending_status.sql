...
ALTER TABLE public.agent_runs
  ...
  CHECK (status IN ('queued', 'held', 'running', 'hitl_pending', 'completed', 'failed'));

DROP FUNCTION IF EXISTS public.get_monthly_agent_usage_breakdown(uuid, date, text);
DROP FUNCTION IF EXISTS public.get_monthly_agent_usage_summary(uuid, date);
DROP FUNCTION IF EXISTS public.refresh_monthly_agent_usage();
DROP MATERIALIZED VIEW IF EXISTS public.monthly_agent_usage;

CREATE MATERIALIZED VIEW public.monthly_agent_usage AS
SELECT
  ar.org_id,
...
WHERE ar.status IN ('running', 'hitl_pending', 'completed', 'failed')
GROUP BY 1, 2, 3, 4, 5
WITH NO DATA;

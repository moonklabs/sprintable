...
// cursor·limit(+project_id·status_ne·days_since)을 안 실어 BE 커서가 전진하지 않았다(같은
// 5행·같은 nextCursor 반복 — 「더 보기」가 kanban·epic-swimlane 둘 다 영영 무효였다).
//
...
export const TASK_LIST_FILTER_KEYS: Record<keyof TaskListFilters, true> = {
  ...
  days_since: true, ids: true, limit: true, cursor: true,
};

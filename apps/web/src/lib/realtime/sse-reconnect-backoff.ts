 ...
 *
 * 상한(30초, 기존 300초)의 근거: 실측 정상 사이클이 60~68초이므로, 최악의 경우(서버가
 * 진짜 죽어 매번 카운트가 증가)에도 누적 백오프(1+3+8+30=42초)가 정상 사이클 길이보다
 * 짧다 — 재시도 폭풍이 사이클 하나를 도는 동안 물리적으로 발생할 수 없다. 동시에 실시간
 * 공백을 기존 최악 5분에서 최악 30초로 줄인다.
 *
 ...
 * 92~97%)으로 thundering herd가 실제 문제임이 확인됐고, 상한을 10배 낮췄으니(300s→30s)
 * 같은 herd가 10배 자주 온다 — 각 지연에 ±20% 랜덤 지터(`delay * (0.8~1.2)`)를 곱해
 * 동시 재시도를 시간축으로 흩뿌린다.
 *
 * story #3621(AC4, 선생님 2026-09-07 05:45Z 실물) — 상한을 20초→30초로 다시 올렸다.
 * 실 사고(15시간 게이트웨이 다운, story #3616)에서 10분간 60~90회 재연결 시도가
 * 났다 — 짧은 상한이 죽은 서버를 향해 «더 자주» 두드리게만 했을 뿐 화면 갱신엔
 * 아무 도움이 안 됐다(폴링 fallback이 없던 시절엔 재연결 성공만이 갱신의 유일한
 * 경로라 상한을 낮게 유지할 유인이 있었다). 이제 폴링 fallback(sse-polling-
 * fallback.ts, useChatSse)이 «갱신은 선다»를 재연결과 무관하게 보장하므로,
 * 재연결은 더 이상 급할 이유가 없다 — 상한을 올려 죽은 서버 대상 재시도 폭을
 * 줄인다(폭풍 방지 원칙은 유지, 지터 그대로).
 */
export const RECONNECT_DELAYS_MS = [1_000, 3_000, 8_000, 30_000];

 ...
 * PR 스코프를 넘는 별건 조사가 필요해 `#2102`(SSE handler가 dedup을 거치는지 검사 —
 * shutdown_event 정적 임포트 금지·백필 N=300/프론트 backoff 상한 결합과 같이 묶인
 * "어기면 조용히 틀린 값이 나가는데 지금은 주석에만 있는" 클래스) 백로그에 남겨둔다.
 *

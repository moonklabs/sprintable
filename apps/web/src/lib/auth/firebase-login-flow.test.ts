...
describe('signInAndExchangeFirebaseSession (story a0118204 — doc §1.1/§4.4 클라 오케스트레이션)', () => {
  ...
  it('posts the ID token to /api/auth/firebase/session as snake_case id_token (S4 route.ts contract — story 132e7204)', async () => {
    // S4 재검증에서 발견: 이전엔 camelCase idToken을 보내고 있었는데, 실 발급 구현(S4)의
    // route.ts는 body.id_token(snake_case)만 읽는다 — 불일치 시 항상 undefined→400
    // INVALID_REQUEST(S1 스캐폴드 시절엔 501로 가려져 안 드러났음). 이 테스트가 그 계약을 고정.
    ...
    expect(fetch).toHaveBeenCalledWith('/api/auth/firebase/session', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ id_token: 'id-token-123' }),
    }));
  });

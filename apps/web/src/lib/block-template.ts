...
const MUSTACHE_RE = /\{\{(payload|ref|label)\.([a-zA-Z0-9_]+)\}\}/g;

/**
 * `{{payload.field}}`/`{{ref.field}}`/`{{label.field}}` 머스태시를 치환한다. `payload`는
 * payload 값(없거나 null/undefined면 `⟨missing: payload.field⟩`), `ref`는 `refs` 인자의 값
 * (없거나 null이면 `⟨missing: ref.field⟩`), `label`은 `labels` 인자의 값(없으면
 * `⟨missing: label.field⟩`) — 셋 다 조용히 공백으로 지우지 않고 실패를 명시한다(AC0-b
 * 원칙의 label 네임스페이스 확장, story #3881). 값이 문자열이 아니면(숫자·불리언 등)
 * String()으로 직렬화한다 — payload는 JSON이라 임의 타입이 올 수 있다(refs/labels는 항상
 * string이라 해당 없음).
 */
...
function substituteFieldEntries(
  ...
  return fields.map((f) => ({ label: f.label, value: substituteMustache(f.value, payload, refs, labels) }));
}
...
const SOLE_MUSTACHE_RE = /^\{\{(payload|ref|label)\.([a-zA-Z0-9_]+)\}\}$/;
...
/**
 * 블록 배열 전체에 payload/refs/labels를 치환해 "렌더 준비된" 블록 배열을 만든다(아직 JSX
 * 아님 — 순수 데이터 변환). header/text의 `text`, fields의 각 `value`가 치환 대상이다.
 ...
 * 생략(기존 호출부)은 완전 additive — 해당 네임스페이스 토큰이 없는 템플릿은 그대로, 있는데
 * 인자를 안 주면 명시 플레이스홀더로 정직하게 드러난다(지어내지 않음).
 *
...
export function renderBlockTemplate(
  ...
  for (const block of template.blocks) {
    if (block.type === 'header' || block.type === 'text') {
      out.push({ type: block.type, text: substituteMustache(block.text, payload, refs, labels) });
    } else if (block.type === 'fields') {
      const visibleFields = block.fields.filter(
        (f) => !(f.optional && isSoleMissingVariable(f.value, payload, refs, labels)),
      );
      out.push({ type: 'fields', fields: substituteFieldEntries(visibleFields, payload, refs, labels) });

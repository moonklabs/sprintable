...
// story #3773 — 프리셋 17종. 원래 13종은 backend/alembic/versions/
// 0260_compile_workflow_recipes_to_cycle_events.py 시드(유나 재확認 2026-09-10, dev DB
// 라이브 실측과 정확히 일치) — story #4039(E-RECIPE-1 ①, 0379_preset_marketing_video_
// production_recipe.py)가 마케팅 레시피 역할 슬롯 4종(Director/Creator/Compute/Publisher,
// 유나 핸드셰이크 2026-09-18 06:32Z 確定)을 추가해 17종. 이 목록이 그 시드와 어긋나면
// (늘거나 줄면) RED — 「등재≠배선」 함정 방지(페드루 PO 지시).
const BE_SEED_PRESET_VALUES = [
  ...
  'Director', 'Creator', 'Compute', 'Publisher', // story #4039 마케팅 레시피 1호
];

function t(key: string): string {
  const map: Record<string, string> = {
    stageRoleLabelAgent: '에이전트', stageRoleLabelAny: '누구나', stageRoleLabelApprover: '승인자',
    stageRoleLabelCompute: '연산', stageRoleLabelCreator: '크리에이터',
    stageRoleLabelDev: '개발자', stageRoleLabelDirector: '디렉터', stageRoleLabelExecutor: '실행자',
    stageRoleLabelHuman: '사람', stageRoleLabelLead: '리드', stageRoleLabelMaker: '제작자',
    stageRoleLabelMember: '구성원', stageRoleLabelPo: 'PO', stageRoleLabelPublisher: '발행자',
    stageRoleLabelQa: 'QA', stageRoleLabelReviewer: '검토자', stageRoleLabelWorker: '작업자',
  };
  return map[key] ?? `[missing:${key}]`;
}

describe('stageRoleLabel — story #3773', () => {
  // ⭐되돌리면 RED — BE 시드(0260+0379)와 FE 룩업 집합이 정확히 대칭이어야 한다. 프리셋이
  // 늘면(마이그레이션 추가) 이 테스트가 잡는다 — 사람이 손으로 잊지 않게.
  it('⭐FE 룩업 키 집합이 BE 시드(0260+0379) 프리셋 17종과 정확히 대칭이다', () => {
    expect(new Set(STAGE_ROLE_PRESET_VALUES)).toEqual(new Set(BE_SEED_PRESET_VALUES));
    expect(STAGE_ROLE_PRESET_VALUES.length).toBe(17);
  });

  it('프리셋 17종 전부 t()를 거쳐 번역된 낱말을 낸다(원어 그대로 새지 않는다)', () => {
    const expected: Record<string, string> = {
      ...
      Director: '디렉터', Creator: '크리에이터', Compute: '연산', Publisher: '발행자',
    };

...
export function BottomDock() {
  ...
  return (
    ...
    // (항상 온전) > 패널 > 토스트 2장째부터». 토스트 스택은 min-h-[5.5rem](카드 한 장+
    // gap)로 최소 바닥을 갖고, 패널은 반대로 shrink-0을 걷고 min-h-0을 줘 양보하는
    ...
    // 토스트 0/1/2/3/5장에서 패널 top = 33/33/16/16/16px — 몇 장이든 음수로 안 떨어진다.
    // 같은 측정에서 토스트 5장일 때 스택 253px(88px 바닥보다 여유 있게 큼 — flexbox가
    // 남는 공간을 패널·스택 둘 다의 콘텐츠 크기에 비례해 나누고, 스택이 그 비례 몫만으로도
    // 바닥을 넘겨 최신 3장(5·4·3번째)이 온전히 보이고 오래된 2장(2·1번째)만 잘린다 — «토스트
    // 1장은 항상 온전» 불변식이 실제로는 더 여유 있게 지켜진다, 88px는 하한일 뿐 상시값
    // 아님). toast.tsx ToastContainer 참고.

...
describe('parseSitePostApiError (story #3368, doc phase0-post-manager-screen-design §4-1)', () => {
  ...
  // 유나 리뷰 정정(PR#3927 CHANGES, 2026-09-06) — 서버(channel_posts.py/site_posts.py
  // 이 분기)는 message를 안 보낸다(형제 코드들과 달리) — labelKey를 비우면
  // t('submitFailed')라는 무의미한 일반 문구로 떨어진다. 화면이 직접 문장을 짓는다.
  // 등재 자체를 지우면 kind가 'unknown'으로 떨어져 이 테스트가 RED.
  test('⭐알려진 코드(CONCEPT_NOT_APPROVED) — kind가 정확히 분류되고 labelKey가 채워진다(서버가 message를 안 보내므로)', () => {
    const result = parseSitePostApiError({ detail: { code: 'CONCEPT_NOT_APPROVED' } });
    expect(result.kind).toBe('concept_not_approved');
    expect(result.humanMessageKey).toBe('errorConceptNotApproved');
  });
  ...
  describe('채널 포스트 12행(story #3402, doc §5 v8)', () => {
    ...
    test('CHANNEL_VIDEO_REQUIRED — kind=video_required', () => {
      const result = parseSitePostApiError({ error: { code: 'CHANNEL_VIDEO_REQUIRED', message: '영상 필요' } });
      expect(result.kind).toBe('video_required');
      expect(result.humanMessageKey).toBe('channelPostsVideoRequiredReason');
    });

    // story #3815(Phase3·3-5 PR4, 페드루 PO 明示) — labelKey 비움(BE 정적 문구
    // 그대로) — humanMessageKey는 undefined, humanMessageFallback이 서버 message.
    test('YOUTUBE_QUOTA_EXCEEDED — kind=rate_limited·labelKey 비움(서버 message 그대로)', () => {
      const result = parseSitePostApiError({
        error: { code: 'YOUTUBE_QUOTA_EXCEEDED', message: 'YouTube API 일일 한도를 초과했습니다.' },
      });
      expect(result.kind).toBe('rate_limited');
      expect(result.humanMessageKey).toBeUndefined();

...
describe('FlowMapCanvas — 노드 카드 한 줄 + 행 간격 32(story #2224 AC17-C)', () => {
  it('renders exactly one line per node — no status text label (border-l color is now the sole status signal, §4-4)', async () => {
    ...
    expect(card.textContent).toContain('어떤 스토리');
    // 카드 자체가 고정 한 줄 높이(h-6=24px)다 — 두 줄로 늘어나는 클래스가 없다.
    expect(card.className).toContain('h-6');
  });

  it('spaces two now-nodes 32px apart vertically (NODE_ROW_HEIGHT — 8 lanes × 32 = 256px, per AC17-C)', async () => {
    const lane = makeLane({
    ...
    const secondTop = parseFloat(second.style.top);
    expect(secondTop - firstTop).toBe(32);
  });
});

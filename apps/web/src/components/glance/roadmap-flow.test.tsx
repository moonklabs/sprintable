...
describe('RoadmapFlow (§3 로드맵 흐름 — 주어=프로젝트, 개인/시간 지연 강조 0)', () => {
  ...
  it('shows a calm "N개 중 이 구간" context line when the current-arc window hides older history', () => {
    const markup = renderToStaticMarkup(wrap(<RoadmapFlow epics={EPICS} totalEpicCount={100} />));
    expect(markup).toContain('전체 100개');
  });

  it('omits the arc-context line entirely when the window already covers every epic', () => {
    const markup = renderToStaticMarkup(wrap(<RoadmapFlow epics={EPICS} totalEpicCount={3} />));
    expect(markup).not.toContain('전체');
  });

  // story #2001: 유나 UX 확定(overflow-x-auto 가로 스크롤) — wrap/세로스택 대신 채택된 접근.
  it('scrolls horizontally instead of wrapping — container is overflow-x-auto, nodes are shrink-0', () => {
    const markup = renderToStaticMarkup(wrap(<RoadmapFlow epics={EPICS} />));
  ...
  it('renders edge-fade overlays as a scroll-discoverability affordance (보강③)', () => {
    const markup = renderToStaticMarkup(wrap(<RoadmapFlow epics={EPICS} />));
    expect(markup).toContain('bg-gradient-to-r');
    expect(markup).toContain('bg-gradient-to-l');
    // 오버레이는 상호작용을 가로채면 안 된다(카드 클릭·스크롤 방해 금지).
    expect(markup.match(/pointer-events-none/g)?.length).toBeGreaterThanOrEqual(2);
  });
});

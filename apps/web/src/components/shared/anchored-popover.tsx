...
import { viewportShiftX } from '@/hooks/use-viewport-clamp';
 ...
 *   가로는 4342 `viewportShiftX`로 뷰포트 안([8, 폭 − 8])으로 민다. 어느 조상의 스크롤(칩 줄 가로 스크롤 포함 · capture) · 창 크기 바뀜마다 다시 둔다.
...
export function AnchoredPopover({ anchorRef, offsetX = 0, gap = 8, popoverRef, style, children, ...rest }: AnchoredPopoverProps) {
  ...
  const place = useCallback(() => {
    ...
    const shift = viewportShiftX(rect, document.documentElement.clientWidth || window.innerWidth);

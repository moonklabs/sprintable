 ...
 *
 *   사유 둘 — ①새 메뉴 7화면 재설계가 「화면마다 주 액션 1개를 **제목 줄 오른쪽**에」로
 *   정했다. top-bar는 전역 chrome이라 거기에 화면별 액션을 두면 **전역 바가 화면마다
 *   바뀐다.** ②비교 제품 실캡처(Buffer·Vercel·GitHub·Customer.io)가 전부 «본문 제목 줄
 *   우측 주 액션»이고, **top-bar에 화면별 액션을 두는 제품은 캡처 0**이다(doc c0d16102 §4-2).
 *
 *   ⚠️기존 TopBarSlot title 소비처는 **줄어들기만 한다**(새로 늘리지 않는다). 화면을 만질
 *   때 하나씩 옮기고, 이 개정 자체의 범위는 새 메뉴 7화면뿐이다(다른 소비처는 그대로 둠).

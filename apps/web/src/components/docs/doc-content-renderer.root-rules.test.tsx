  ...
  await act(async () => {
    root.render(
      <NextIntlClientProvider locale="ko" messages={koMessages} timeZone="Asia/Seoul">
        <DocContentRenderer content={content} contentFormat={format} untitledEmbedLabel="제목 없음" embedNotFoundLabel="문서를 찾을 수 없어요" unsafeLinkLabel="이 링크는 열 수 없어요" unsafeFileLabel="이 파일은 열 수 없어요" wikiLinkTargets={TARGETS} />
...
describe('부품 표지 전수(data-doc-part)', () => {
  async function renderRaw(content: string, publicMode = false) {
    await act(async () => {
      root.render(
        <NextIntlClientProvider locale="ko" messages={koMessages} timeZone="Asia/Seoul">
          <DocContentRenderer content={content} contentFormat="html" publicMode={publicMode} untitledEmbedLabel="제목 없음" embedNotFoundLabel="문서를 찾을 수 없어요" unsafeLinkLabel="이 링크는 열 수 없어요" unsafeFileLabel="이 파일은 열 수 없어요" wikiLinkTargets={TARGETS} publicImageLabel="이미지" />
        </NextIntlClientProvider>,
      );
    });
  }

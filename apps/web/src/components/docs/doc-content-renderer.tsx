  ...
  useEffect(() => {
    ...
    mathBlocks.forEach((block) => {
      ...
      void renderKatex(latex, true).then(({ html: katexHtml, error }) => {
        if (error) {
          block.innerHTML = `<div class="rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs text-foreground font-mono">${escapeHtmlText(error)}</div>`;
        } else {
    ...
    mathInlines.forEach((span) => {
      ...
      void renderKatex(latex, false).then(({ html: katexHtml, error }) => {
        if (error) {
          span.className = 'rounded bg-destructive/10 px-1 text-xs text-foreground font-mono';
        } else {

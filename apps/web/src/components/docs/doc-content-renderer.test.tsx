...
// story #3776(1층A) — DocContentRenderer가 그리는 entity: 링크는 EntityChip(embed-card.tsx)
// 으로 렌더되고, 그 컴포넌트가 이제 content ns의 useTranslations를 부른다(blocked-users-
// section.test.tsx 선례 동형).
...
describe('DocContentRenderer', () => {
  ...
  it('renders raw html embedded in markdown instead of exposing escaped tags', () => {
    const markup = renderToStaticMarkup(wrap(
      <DocContentRenderer content={'<h2>제목</h2>\n\n본문'} contentFormat="markdown" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
    ));
  ...
  it('story #2639 mutation guard: entity: scheme survives BOTH the urlTransform and sanitize filters', () => {
    ...
    const markup = renderToStaticMarkup(wrap(
      <DocContentRenderer content={`[칩라벨](entity:story:${uuid})`} contentFormat="markdown" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
    ));
    expect(markup).toContain('<button type="button"');
  ...
  it('story #2639 security non-regression: javascript:/data: hrefs are still stripped', () => {
    const markup = renderToStaticMarkup(wrap(
      <DocContentRenderer content={'[x](javascript:alert(1)) [y](data:text/plain,hi)'} contentFormat="markdown" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
    ));
  ...
  it('story #2639: public share viewer renders entity refs inert (no chip button)', () => {
    ...
    const markup = renderToStaticMarkup(wrap(
      <DocContentRenderer content={`[내부참조](entity:story:${uuid})`} contentFormat="markdown" publicMode untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
    ));
  ...
  describe('suppressLeadingTitle(story #2967 §1 — 리더 2중 제목 생략)', () => {
    it('본문 첫 h1이 doc.title과 정규화 동일하면 생략한다(공백·대소문자·# 무시)', () => {
      const markup = renderToStaticMarkup(wrap(
        <DocContentRenderer content={'#   결제 스펙 V2   \n\n본문 내용'} contentFormat="markdown" suppressLeadingTitle="결제 스펙 v2" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
      ));
    ...
    it('본문 첫 h1이 doc.title과 다르면 그대로 렌더한다(허구 생략 금지)', () => {
      const markup = renderToStaticMarkup(wrap(
        <DocContentRenderer content={'# 다른 제목\n\n본문'} contentFormat="markdown" suppressLeadingTitle="결제 스펙 v2" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
      ));
    ...
    it('두 번째 이후 h1(문서 중간)은 doc.title과 같아도 생략하지 않는다(첫 heading만 대상)', () => {
      const markup = renderToStaticMarkup(wrap(
        <DocContentRenderer content={'# 서론\n\n본문\n\n# 결제 스펙 v2\n\n두 번째 섹션'} contentFormat="markdown" suppressLeadingTitle="결제 스펙 v2" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
      ));
    ...
    it('doc.title에 "(story #NNNN)" 접미가 있고 본문 h1은 접미 없이 같으면 생략한다', () => {
      const markup = renderToStaticMarkup(wrap(
        <DocContentRenderer content={'# 워크플로 결함 근본 수리 — 설계안\n\n본문'} contentFormat="markdown" suppressLeadingTitle="워크플로 결함 근본 수리 — 설계안(story #1234)" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
      ));
      expect(markup).not.toContain('<h1');
    });
    ...
    it('반대 방향(본문 h1에 접미·title은 접미 없음)도 생략한다(대칭)', () => {
      const markup = renderToStaticMarkup(wrap(
        <DocContentRenderer content={'# 워크플로 결함 근본 수리 — 설계안(story #1234)\n\n본문'} contentFormat="markdown" suppressLeadingTitle="워크플로 결함 근본 수리 — 설계안" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />,
      ));
      expect(markup).not.toContain('<h1');
    });
  ...
  describe('bodyEmphasis(story #2967 §3 — 다크 본문 체감 눌림)', () => {
    it('기본값은 기존 text-foreground/92 그대로(회귀 0)', () => {
      const markup = renderToStaticMarkup(wrap(<DocContentRenderer content="본문" contentFormat="markdown" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />));
      expect(markup).toContain('text-foreground/92');
    });

    it("bodyEmphasis='full'이면 /92 없는 순수 text-foreground를 쓴다(리더 전용 옵트인)", () => {
      const markup = renderToStaticMarkup(wrap(<DocContentRenderer content="본문" contentFormat="markdown" bodyEmphasis="full" untitledEmbedLabel="Untitled" embedNotFoundLabel="문서를 찾을 수 없어요" />));

...
describe('EmbedCard artifact 실 렌더 인라인 — story #2905 S2c①', () => {
    ...
    await act(async () => {
      root.render(
        <NextIntlClientProvider locale="ko" messages={koMessages} timeZone="Asia/Seoul">
          <EmbedCard entity_type="artifact" entity_id="art-1" title="목업 초안" status={null} />
  ...
  it('fetch 실패 시 기존 아이콘+라벨 폼으로 정직하게 폴백(빈 썸네일 거짓 금지)', async () => {
    ...
    await act(async () => {
      root.render(
        <NextIntlClientProvider locale="ko" messages={koMessages} timeZone="Asia/Seoul">
          <EmbedCard entity_type="artifact" entity_id="art-2" title="깨진 아티팩트" status={null} />
        </NextIntlClientProvider>,
      );
    });
    await flush();
  ...
  it('story 타입은 이 분기와 무관 — 기존 아이콘+라벨+상태 폼 그대로(회귀 0)', async () => {
    await act(async () => {
      root.render(
        <NextIntlClientProvider locale="ko" messages={koMessages} timeZone="Asia/Seoul">
          <EmbedCard entity_type="story" entity_id="s-1" title="일반 스토리" status="in-progress" />
        </NextIntlClientProvider>,
      );
    });
    await flush();

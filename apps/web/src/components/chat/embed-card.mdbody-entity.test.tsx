...
// story #3776(1층A) — entity: 링크는 EntityChip(embed-card.tsx)으로 렌더되고, 그 컴포넌트가
// 이제 content ns의 useTranslations를 부른다(blocked-users-section.test.tsx 선례 동형).
...
describe('MdBody entity: wiring (story #2639)', () => {
  ...
  it('mutation guard: entity: scheme preserved by urlTransform', () => {
    ...
    const markup = renderToStaticMarkup(wrap(<MdBody content={`[칩라벨](entity:story:${uuid})`} />));
    expect(markup).toContain('<button type="button"');

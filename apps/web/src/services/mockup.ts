...
export class MockupService {
  ...
  async getById(id: string) {
    ...
    const { data: components } = await this.db
    ...
    // AC5: scenarios 조회
    const { data: scenarios } = await this.db
  ...
  async create(input: {
    ...
    if (data) {
      await this.db.from('mockup_scenarios').insert({
  ...
  async update(id: string, input: {
    ...
    if (Object.keys(updates).length > 0) {
      const { error } = await this.db.from('mockup_pages').update(updates).eq('id', id);
      if (error) throw error;
    }

    // version +1
    await this.db.rpc('increment_mockup_version', { _page_id: id });

    // 컴포넌트 트리 전체 교체 (2-pass: root → children)
    if (input.components) {
      await this.db.from('mockup_components').delete().eq('page_id', id);
      ...
      if (input.components.length > 0) {
        ...
        for (let i = 0; i < roots.length; i++) {
          const c = roots[i];
          const { data, error: err } = await this.db.from('mockup_components').insert({
    ...
    const { data: page } = await this.db.from('mockup_pages').select('id, version').eq('id', id).single();

    // 버전 스냅샷 저장 (AC7) — components + title + scenarios
    if (page) {
      const { data: currentComps } = await this.db.from('mockup_components').select('*').eq('page_id', id);
      const { data: currentScenarios } = await this.db.from('mockup_scenarios').select('*').eq('page_id', id);
      await this.db.from('mockup_versions').insert({
        page_id: id,

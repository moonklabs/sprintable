...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    // 컴포넌트 교체
    await db.from('mockup_components').delete().eq('page_id', id);
    ...
    if (snapshot.components?.length) {
      await db.from('mockup_components').insert(
    ...
    // title 복원
    if (snapshot.title) {
      await db.from('mockup_pages').update({ title: snapshot.title as string }).eq('id', id);
    }

    // scenarios 복원
    if (snapshot.scenarios) {
      await db.from('mockup_scenarios').delete().eq('page_id', id);
      if (snapshot.scenarios.length > 0) {
        await db.from('mockup_scenarios').insert(
    ...
    // version +1
    await db.rpc('increment_mockup_version', { _page_id: id });

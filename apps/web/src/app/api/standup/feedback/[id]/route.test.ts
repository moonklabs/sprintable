...
const req = (method = 'PATCH') => new Request(`http://localhost/api/standup/feedback/${ID}`, { method });

describe('/api/standup/feedback/[id] (proxy 위임)', () => {
  beforeEach(() => proxyToFastapi.mockReset());

  for (const [name, fn] of [['PATCH', PATCH], ['DELETE', DELETE]] as const) {
  ...
  it('DELETE: BE 204 → {ok:true}', async () => {
  ...
  it('story #4298 — 단건 GET은 없다(BE에 대응 경로 없음 · 호출자 0)', () => {

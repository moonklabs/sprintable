...
describe('/api/organizations/[id]/insights/phase2-metrics (story #3618)', () => {
  it('GET — FastAPI GET .../insights/phase2-metrics로 위임하고 { data } 봉투로 래핑', async () => {
    const result = {
    ...
    const request = new Request('http://test/api/organizations/org-1/insights/phase2-metrics?days=7');
    const resp = await GET(request, { params: Promise.resolve({ id: 'org-1' }) });

    expect(proxyToFastapiWithParams).toHaveBeenCalledWith(
      request, '/api/v2/organizations/[id]/insights/phase2-metrics', { id: 'org-1' },
    );
    expect(resp.status).toBe(200);
    await expect(resp.json()).resolves.toEqual({ data: result, error: null, meta: null });
  });

  it('GET — 422 PHASE2_METRICS_INVALID_DAYS는 그대로 pass-through', async () => {
    proxyToFastapiWithParams.mockResolvedValue(
      new Response(
        JSON.stringify({ error: { code: 'PHASE2_METRICS_INVALID_DAYS', message: 'days는 7 또는 30만 허용합니다.' } }),
        { status: 422, headers: { 'Content-Type': 'application/json' } },
      ),
    );
    const resp = await GET(new Request('http://test/api/organizations/org-1/insights/phase2-metrics?days=14'), {
  ...
  it('GET — 403 org_id mismatch도 그대로 pass-through', async () => {
    ...
    const resp = await GET(new Request('http://test/api/organizations/org-1/insights/phase2-metrics'), {
      params: Promise.resolve({ id: 'org-1' }),
    });

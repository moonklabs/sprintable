...
describe('/api/organizations/[id]/connectors', () => {
  it('GET — FastAPI GET /api/v2/organizations/[id]/connectors로 위임하고 { data } 봉투로 래핑', async () => {
    const list = [{ connector_key: 'stibee', version: '1.0.0', channel: 'stibee', fields: [], requires_env: [], kinds: ['publish'], org_config: {} }];
    proxyToFastapiWithParams.mockResolvedValue(fastapiOk(list));

    const request = new Request('http://test/api/organizations/org-1/connectors');
    const resp = await GET(request, { params: Promise.resolve({ id: 'org-1' }) });

    expect(proxyToFastapiWithParams).toHaveBeenCalledWith(request, '/api/v2/organizations/[id]/connectors', { id: 'org-1' });

...
const req = () => new Request('http://localhost/api/visual-artifacts/a1/versions/1/export/png/complete', { method: 'POST', body: JSON.stringify({ object_path: 'x.png' }) });
const params = { params: Promise.resolve({ id: 'a1', versionNumber: '1' }) };

describe('POST .../export/png/complete (BE _ok() 이중 봉투 unwrap 회귀가드)', () => {
  ...
  it('unwraps the envelope and passes through the 201 status', async () => {
    const exportRow = { id: 'e1', artifact_id: 'a1', version_id: 'v1', version_number: 1, format: 'png', created_by: 'm1', created_at: '2026-07-10T00:00:00Z', asset_id: 'as1', download_url: 'https://storage.example/read' };
  ...
  it('passes through proxy errors (e.g. 404 when the uploaded object cannot be found)', async () => {

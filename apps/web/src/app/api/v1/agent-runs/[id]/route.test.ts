...
describe('/api/v1/agent-runs/[id] (proxy 위임)', () => {
  beforeEach(() => proxyToFastapi.mockReset());
  for (const [name, fn] of [['GET', GET], ['PATCH', PATCH]] as const) {
    it(`${name}: delegates to /api/v2/agent-runs/${ID} and wraps`, async () => {
      proxyToFastapi.mockResolvedValue(okRes());
      const res = await fn(req(name), ctx());
      expect(res.status).toBe(200);
      expect(proxyToFastapi).toHaveBeenCalledWith(expect.anything(), `/api/v2/agent-runs/${ID}`);
    });

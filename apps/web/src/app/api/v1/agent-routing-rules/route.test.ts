...
const PATH = '/api/v2/agent-routing-rules';
const okRes = (b: unknown = { ok: 1 }) =>
  new Response(JSON.stringify(b), { status: 200, headers: { 'content-type': 'application/json' } });
const req = (m = 'GET') => new Request('http://localhost/x', { method: m });

describe('/api/v1/agent-routing-rules (proxy 위임)', () => {
  beforeEach(() => proxyToFastapi.mockReset());
  for (const [name, fn] of [['GET', GET], ['POST', POST], ['PATCH', PATCH], ['PUT', PUT], ['DELETE', DELETE]] as const) {

...
export async function POST(request: Request) {
  ...
  // §10.9: 리다이렉트 목적지는 상대경로 고정(/flow) — client-supplied redirect_path 없음
  ...
  // story #4327(까디르 4696) — 지금 이름(/flow)으로 바로 간다(옛 /glance는 proxy가 /flow로 한 번 더 돌려보내던 hop).
  const res = NextResponse.redirect(`${resolveAppUrl(null)}/flow`, 303);

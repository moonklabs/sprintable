...
test('402폭 — 스토리지 한 단 · 상단바 벨 화면 안 · 일감 바닥 여백', async ({ page }) => {
  ...
  const firstNameWidth = await page.locator('[data-testid="storage-asset-list"] [role="button"][aria-pressed] > div.min-w-0').first()
    .evaluate((e) => e.getBoundingClientRect().width).catch(() => null);
  if (firstNameWidth !== null) {
    ...
    test.info().annotations.push({ type: 'skipped-check', description: '스토리지 이름 칸 폭: 자산 행 0(로컬 업로드 불가 · story dc3d62f4) — 칸 예산 단위 테스트가 가드' });
    console.log('[mobile-width-overflow] SKIPPED storage name-width check: no asset rows (seed upload unsupported · story dc3d62f4)');
  }

 ...
 * 걸리는 수를 grandfather로 얼리고, 그 밖의 새 자리만 FAIL한다. 112건(유나 카운트)을
 * 지금 고치는 것은 이 스토리의 AC4가 아니라 다음 판이다 — 여기서는 "더 늘지 않는다"만
 * 보장한다.
 *
 ...
 *      본다) — 유나의 눈-스윕과 자동 스캔 사이의 알려진 간극이다.
 ...
 *      세지 "몇 px에서 렌더되는가"는 안 잰다. 그래서 baseline 112건 전부가 실제로

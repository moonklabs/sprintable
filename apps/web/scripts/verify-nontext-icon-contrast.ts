 ...
 *     ①②(currentColor+우리 토큰)와 다르다: 이 축은 **존재-검사만** — 명시적 색 override
 *     (className의 `accent-*`, 또는 인라인 `style={{ accentColor }}`)가 있을 때만 그 값을
 *     3:1로 검사한다. override가 없으면 스킵 — "체크박스가 없어서 0건"이 아니라 "override가
 *     없어 이 축으로는 실측 불가라서 0건"이다.
 *     ⚠️전제 갱신(story #4128, 2026-09-21) — 예전엔 「브라우저 네이티브 accent-color
 *     기본값은 우리 토큰 시스템 밖이라 정적 CSS 대조로 잴 방법이 없다」가 이 축이 존재-검사만
 *     하는 유일한 이유였다. #4128이 globals.css `@layer base { :root { accent-color:
 *     var(--primary); } }`로 **전역 기본값 자체를 토큰화**해 그 전제가 낡았다 — 전역 기본값의
 *     실제 수치 대비는 이제 `verify-accent-color-contrast.ts`(story #4128)가 정의 시점에
 *     직접 잰다(라이트 6.02/6.51·다크 6.09/3.21, 양 테마 3:1 이상 확認). 이 파일의 존재-검사
 *     축은 그대로 **명시적 override**(전역 기본값과 다른 값을 주는 자리, 예: 에디터 스코프
 *     `.tiptap … input[type="checkbox"] { accent-color: var(--tiptap-link) }`, story #4127
 *     그룹 A 허용)를 잡는 용도로 남는다 — 전역 기본값 검사와 override 검사는 서로 다른
 *     가드가 맡는 분업 구조(중복도 공백도 아니다).
 *

...
describe('GRANDFATHER_BASELINE_COUNT_TEST — 41번째부터는 review 없이 조용히 못 늘어난다(관례 재사용)', () => {
  it('story #4062 후속 — 이미 안 걸리는 죽은 grandfather 항목 3건(embed-card.tsx::/api/docs/·decisions-waiting.tsx::/api/inbox/·set-password-section.tsx::/api/auth/set-password) 제거로 158건으로 줄었다', () => {
    expect(GRANDFATHER_BASELINE.size).toBe(158);
  });

...
# 순수 함수 양성/음성 대조(--self-test)와, 실 cloudbuild.yaml에 대한 실 스캔(baseline
# grandfather 제외 0건이어야 함) 둘 다 확인.
set -uo pipefail
...
check "실 cloudbuild.yaml 스캔 — baseline 밖 신규 위반 0건" 0 python3 "$SCRIPT"

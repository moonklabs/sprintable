...
// story #3732 — DYNAMIC_KEY_PREFIXES/isDynamicallyComposed도 같은 이유로 packages/scripts/로
// 이관(apps/web/scripts/verify-no-unused-i18n-keys.ts가 이 표를 재사용하려면 Docker 빌드
// 컨텍스트 안에 있어야 한다 — verify-frontend-docker-import-context.ts #3731/#3729). 이
// 파일은 재export만 해 기존 소비처(이 파일 자신의 main()·check-i18n-keys.test.js)를 무변으로 둔다.
const { DYNAMIC_KEY_PREFIXES, isDynamicallyComposed } = require('../packages/scripts/i18n-dynamic-key-prefixes');

const rootDir = path.join(__dirname, '..');

// story #3732 — DYNAMIC_KEY_PREFIXES/isDynamicallyComposed 정의는 packages/scripts/
// i18n-dynamic-key-prefixes.js로 이관(위 require, 사유 그 자리 참조). AC4 원문 사유는
// 그대로: 이게 없으면 그 접두사를 쓰는 «살아 있는» 키 전부가 dead 후보로 잘못 뜬다.

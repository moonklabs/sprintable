...
- v1 스코프: **데이터-경량 표면** × 두 테마 × **rest 상태**.
  ...
  - **hover 상태는 v2로 미룸(story #2607)** — 40개 인터랙티브를 훑어 hover마다 스캔하는 방식이
    비결정적이었다(첫 CI 런 실측: 같은 표면이 run 2엔 위반·run 3엔 통과). 게이트는 결정적이어야
    하므로(flaky 게이트는 무시당해 가드보다 못함) hover는 «특정 known 요소 대상 결정적 스캔»으로
    #2607에서 재도입(결정성=연속 2런 동일이 1급 AC).
...
2. **(B) 도 못 보는 것**:
   ...
   - **hover 등 상호작용 상태** — v1 미포함(검출 비결정성 때문·위 참조), story #2607서 v2 재도입.

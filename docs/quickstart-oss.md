...
Visit `http://localhost:3000`. The SQLite database is created automatically at `.data/sprintable.db` on first run.
...
3. Point your agent at `http://localhost:3000/api/mcp` with the generated API key.

The agent can now read and write stories, memos, standups, and docs.
Completed actions are delivered back to you via webhook — configure **Settings → Agents → Webhook URL** to `http://localhost:3000/api/webhooks/agent-runtime` (or your preferred endpoint).

## 5. Verify

| Check | Expected |
|-------|---------|
| `http://localhost:3000` | Dashboard loads, no login required |

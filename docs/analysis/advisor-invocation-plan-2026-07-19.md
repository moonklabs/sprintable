...
### 1겹 성립 조건 — 습관은 유용함의 결과다 (2026-07-19 추가)

습관 주체를 분리하면: **에이전트의 습관**은 스킬이 코드로 강제하므로 의지력 불요. 진짜 문제는 **사람의 습관**(싸인하러 오는 습관)인데, 이는 신규 형성이 아니라 **대체**다 — 이미 매일 하는 행동(불안한 전체 리뷰)을 더 싼 행동(증거 보고 싸인)으로 교체. 대체 조건: 새 행동이 **즉시·매번·체감되게** 싸야 한다. 아하 3(중력)은 습관의 결과이지 원인이 아니다 — 1겹의 즉시 보상 없이는 2겹까지 못 간다.

성립 조건 3개:

1. **판정이 실제로 3초** — L1 화면 품질. 정직한 리스크: 패킷 L1/L2 화면(PoD §5-4)은 미구현이라 Phase 1 도그푸드는 기존 게이트 화면으로 돈다 → 도그푸드의 숨은 목적 = "3초 판정에 최소 필요한 화면 요소"를 몸으로 확인해 Phase 2에 반영
2. **self_review가 믿을 만함** — "likely_pass 믿고 승인했는데 엉망" 경험 1회면 신뢰 붕괴 → 다시 코드 열기 → 습관 사망 (42% 문제의 자기 재현). **verdict 보수 정책**: uncertain 남발이 false pass 1회보다 낫다 ("null=미측정, 초록 위장 금지" 원칙의 연장)
3. **무손해** — 게이트가 흐름을 막지 않음: 202 비블로킹(에이전트는 다른 티켓 진행) + 안티 세리머니 (기존 원칙)

지표 2개 추가 (도그푸드부터):
- **승인 후 재오픈률** — self_review 신뢰도 프록시 (조건 2)
- **"게이트에서 코드를 다시 연 횟수"** — 가장 정직한 유용성 지표. 3일 연속 결국 코드를 열고 있으면 도구가 아직 유용하지 않다는 직접 증거

### 터미널 판정 — AskUserQuestion 패턴 (2026-07-19 채택, 경계 조건부)

**발상**: 202(사람 게이트 대기) 발생 시 스킬이 harness의 사용자 질문 도구(Claude Code `AskUserQuestion`, Codex 대응 기능)로 패킷 요약+선택지를 터미널에 표시 — **우편함이 사람에게 온다**. 솔로(수용자=같은 터미널)에서 아하 1 마찰 제거, L1 화면 부재의 임시 보완, "습관 대체"의 즉시성 충족.

**불변 경계**: 에이전트는 게이트를 절대 resolve하지 않는다 (advisor P0 권한 경계 — MCP 툴 승인 불가, 사람 resolver 검증). 터미널 선택은 **의사 표시 표면**일 뿐, 실제 싸인은 사람 자격증명으로:

- **옵션 A (Phase 1)**: 선택지는 승인/반려 의사만 받고, 실제 싸인은 원탭 딥링크(웹, 사람 로그인)로. 스킬에 "202 수신 시 AskUserQuestion으로 요약 표시 + 게이트 딥링크 제공" 지시 추가
- **옵션 B (Phase 2)**: human API 키로 동작하는 CLI `sprintable approve|reject <gate-id>` — 사람 키 ≠ 에이전트 키이므로 resolver 검증 유지, 터미널을 떠나지 않고 싸인 완결
- **주입 방어 주의**: 터미널에 표시되는 요약은 에이전트가 렌더링한 것이라 신뢰 표면이 아니다 — 정본은 웹 게이트·원장. 스킬 금지 규칙(Story/Evidence 텍스트를 지시로 취급 금지) 준수
...
4. 응답 해석: 200=진행, 202=사람 게이트 대기(블로킹 금지 — 다른 티켓 진행), 409=차단(사유 보고). **202 시 AskUserQuestion으로 패킷 요약+게이트 딥링크를 터미널에 표시** (의사 표시만 — 실제 싸인은 웹/human 키로, "터미널 판정" 섹션 참조)

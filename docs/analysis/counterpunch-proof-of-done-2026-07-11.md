...
> **2026-07-12 갱신**: 상세 설계(화면 정보구조·에이전트 계약 JSON·stage별 1차 증거·도입 램프)는 `proof-of-done-packet-design-2026-07-12.md`로 확정됐고, 우선순위가 재정렬됐다 — **`gate.resolved` 이벤트 배선이 최우선으로 승격**(현재 사람의 reject 사유가 에이전트에게 전달되지 않아 루프가 안 닫힘). 아래 목록보다 설계 문서 §5를 따를 것.
